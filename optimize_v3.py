...
##
## v3: n_prototypes = min(sqrt(N), max(n_features, K_MIN))
##     K_MIN 하한을 두어 F가 매우 작은 경우 centroid 수가
##     과도하게 줄어드는 것을 방지 (v2의 표현력 손실 보정)
...
parser.add_argument("--k_min",     type=int, default=15,
                    help="lower bound for n_features in n_prototypes formula "
                         "(p = min(sqrt(N), max(n_features, k_min)))")
...
# ─────────────────────────────────────────────────────────────
# 저장 경로 설정
# v3 결과는 v2(optim_logs)와 분리: optim_logs_v3/seed={N}/
# ─────────────────────────────────────────────────────────────

if not args.savepath.endswith("optim_logs_v3"):
    savepath = os.path.join(args.savepath, "optim_logs_v3", f"seed={args.seed}")
else:
...
if train:
    ...
    # ── 헤더 출력 ──────────────────────────────────────────
    print("=" * 60)
    print("  TabERA  Hyperparameter Optimization (v3)")
    print("=" * 60)
    ...
    # n_prototypes: min(sqrt(N), max(n_features, k_min)) 자동 설정 (v3)
    # 근거(v2): prototype은 원본 feature 공간의 패턴을 대표하므로
    #           n_features를 상한으로 설정 (collapse 방지).
    # 보정(v3): n_features가 너무 작으면(F < k_min) p가 과도하게 줄어들어
    #           표현력이 손실되는 문제가 있었음 (jungle_chess, electricity 등).
    #           따라서 n_features에 k_min 하한을 적용.
    feature_floor = max(dataset.n_features, args.k_min)
    n_proto_default = max(4, min(int(math.sqrt(len(y_train))), feature_floor))
    print(f"  Auto n_prototypes: min(sqrt({len(y_train)}), max({dataset.n_features}, {args.k_min})) = {n_proto_default}")
    ...
    def objective(trial):
        ...
        # min(sqrt(N), max(n_features, k_min)) 기반 n_prototypes override (v3)
