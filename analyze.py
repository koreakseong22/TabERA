...
def print_calibration_analysis(result: dict) -> None:
    ...
    # ⚠ This is a tolerance test, not a monotonicity test: each adjacent step
    #   may fall by up to 3%p, so a slow steady decline passes. The wording
    #   below says exactly that and no more.
    no_large_adjacent_drop = all(a <= b + 0.03 for a, b in zip(accs, accs[1:]))
    max_drop = max((accs[i] - accs[i+1] for i in range(len(accs)-1)), default=0.0)

    print(f"\n  Reading:")
    if no_large_adjacent_drop and max_drop < 0.05:
        # ⚠ Observation only. Retrieval is not on the prediction path and there
        #   is no fusion, so this curve cannot be read as either of them
        #   compensating for anything; nor does it single out W_eff·d.
        print(f"    - Accuracy across routing-confidence percentile bins shows no adjacent")
        print(f"      decline larger than 3%p (largest {max_drop:.1%}p): the final predictor")
        print(f"      is robust to variation in routing confidence within that tolerance.")
        print(f"      This analysis alone does not identify the mechanism.")
    else:
...
def run_single_seed(
    ...
    """
    # Work on a per-run copy: an inherited HP must not become an explicit
    # override when this function is called again for another seed.
    import copy
    from libs.search_space import resolve_dynamics, RECIPE_TAG
    args = copy.copy(args)
    _inherit_beta_lr = args.beta_lr_mult is None
    if _inherit_beta_lr:
        args.beta_lr_mult = 1.0
    ...
    if args.from_saved_state:
        ...
        # PLE bin edges travel with the checkpoint (they are also the
        # embedder.ple_edges buffer in its state_dict). The training branch
        # computes them from X_train; this branch used to leave the name
        # unbound, and the unconditional model_kwargs.update(num_bin_edges=…)
        # further down then raised UnboundLocalError before the model was
        # built. Keep the checkpoint's own edges; recompute from the same
        # train split only for old checkpoints that did not store them.
        num_bin_edges = model_kwargs.get("num_bin_edges")
        if (num_bin_edges is None
                and model_kwargs.get("num_embedding", args.num_embedding) == "ple"
                and len(dataset.X_num) > 0):
            ...
            print("  !  old-format file (no num_bin_edges): recomputed from the train split.")
        ...
        # ── Load the study ─────────────────────────────────────
        # ⚠ The source study's geometry and head scale are a pair. With the
        #   final defaults, `--params_geometry additive` alone would look for
        #   an additive study tagged ..hs=auto -- a combination that cannot
        #   exist, since auto is defined only for unit_tangent. Normalise once
        #   here and use the same two values for the lookup and for meta.pkl.
        _src_geometry = (args.params_geometry if args.params_geometry is not None
                         else args.correction_geometry)
        _src_head_input_scale = (
            "unit" if (args.params_variant == "legacy"
                       or args.head_input_scale == "matched"   # matched reads a unit-scale source
                       or _src_geometry == "additive")
        ...
        _study_tag = study_pkl_tag(
            ...
            correction_geometry=_src_geometry,
            ...
            head_input_scale=_src_head_input_scale,
        ...
        if "ema_timescale" in best_params or "beta_lr_mult" in best_params:
            _dynamics = resolve_dynamics(best_params, len(X_train))
            model_kwargs["ema_decay"] = _dynamics["ema_decay_actual"]
            if _inherit_beta_lr:
                args.beta_lr_mult = _dynamics["beta_lr_mult_requested"]
            _save_tag += RECIPE_TAG
            print(f"  recipe dynamics: {_dynamics}; beta multiplier used={args.beta_lr_mult}")
    ...
    _training_provenance = {
        ...
        # Keep provenance source-scale resolution identical to the study
        # lookup: geometry and scale are a pair, and an additive source study
        # is always unit-scale (auto exists only for unit_tangent).
        "params_head_input_scale_source": (
            "unit" if (args.params_variant == "legacy"
                       or actual_head_input_scale == "matched"
                       or (args.params_geometry if args.params_geometry is not None
                           else actual_geometry) == "additive")
            ...
            if refresh_stats is not None:
                ...
                if regroup_stats is not None:
                    print(f"  [--refresh_on_best] resynced sample_groups on the clean "
                          f"embeddings against the saved centroids "
                          f"(active={regroup_stats.get('active_ratio', 0)*100:.0f}%)")
        else:
    ...
    try:
        ...
        _mt_fname = os.path.join(
            _mt_dir, f'model=tabera_analysis{_mt_variant}'
        ...
        if _refine_info.get("enabled") and _refine_info.get("raw_test_metrics"):
            _raw_fname = os.path.join(
                _mt_dir, f'model=tabera_analysis{_mt_variant}_raw'
                         '..init_hps=False..deep=0..hyper=0.npy')
            np.save(_raw_fname, {
    ...
    meta = {
        ...
        "freeze_deviations": {
            ...
            if v != {"n_prototypes": None, "beta_lr_mult": 1.0,
                     ...
                     # ⚠ "Deviation" means deviation from the *final*
                     #   architecture, so the baseline is FINAL_CONFIG -- not
                     #   the legacy literals the filename tag keeps for
                     #   backward-compatible study names. With the literals
                     #   here every normal final run was recorded as deviant.
                     "correction_geometry": FINAL_CONFIG["correction_geometry"],
                     "beta_param": FINAL_CONFIG["beta_param"],
                     "head_input_scale": FINAL_CONFIG["head_input_scale"],
                     "tie_rule": FINAL_CONFIG["tie_rule"],
                     "params_seed": None,
                     "params_geometry": None, "params_variant": None,
                     "early_stop_metric": FINAL_CONFIG["early_stop_metric"]}[k]
        },
    ...
    if args.explain and do_analysis:
        ...
        if not explanations:
            ...
            if args.explain_figure:
                ...
                _stem = Path(args.explain_figure)
                for i in range(n_show):
                    _paths = render_explanation_figure(
                        explanations, i,
                        ...
                        target_class_names=getattr(
                            dataset, "target_class_names", None),
                        tasktype=tasktype,
                        dataset_name=dataset_info.get("fullname", "") or
...
def main():
    ...
    parser.add_argument("--beta_lr_mult", type=float, default=None,
                        help="override beta LR multiplier; default inherits HPO (legacy: 1.0)")
    ...
    parser.add_argument("--head_input_scale", type=str,
    ...
    parser.add_argument("--calibration_analysis", action="store_true",
                        help=(
                            ...
                            "A flat accuracy curve across routing-confidence bins shows "
                            "the final predictor is robust to variation in routing "
                            "confidence -- this analysis alone does not say why; a high "
                            "ECE on prediction confidence -- "
    ...
    parser.add_argument("--correction_geometry", type=str,
